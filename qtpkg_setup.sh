...
rm module || mv module module.$TIMESTAMP                || die
...
rm lib || mv lib lib.$TIMESTAMP                         || die
ln -s ../../lib/ lib                                    || die
...
[ -h lib/module ]      && git update-index --assume-unchanged lib/module
[ -h application/lib ] && git update-index --assume-unchanged application/lib
...
if [ "$QTVER" -eq 5 ] ; then
  ...
  sudo apt-get install -q -y qt4-qmake libqtcore4 libqtgui4 libqtwebkit4        \
                             libqt4-designer libqt4-help libqt4-network         \
